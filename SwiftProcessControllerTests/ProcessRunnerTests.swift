...
final class ProcessRunnerTests: XCTestCase {
	...
	func testRunner() throws {
		let runner = SPCProcessRunner(executablePath: "/bin/sh")
		XCTAssertNoThrow({
	...
	func testRunnerBasicReturnsAndExitStatuses() throws {
		let runTrue = SPCProcessRunner(executablePath: "/usr/bin/true")
		...
		let runFalse = SPCProcessRunner(executableURL: url)
		let resFalse = try runFalse.run(args: [])
		XCTAssertEqual(resFalse.exitStatus, 1)
	}
	
	func testRunnerOutput() throws {
		let run = SPCProcessRunner(executablePath: "/usr/bin/printf")
	...
	func testRunnerStdBoth() throws {
		let run = SPCProcessRunner(executablePath: "/bin/sh")
	...
	func testDefaultEnv() throws {
		let run = SPCProcessRunner(executablePath: "/usr/bin/printenv")
	...
	func testCurrentDirectory() throws {
		let run = SPCProcessRunner(executablePath: "/bin/pwd")
		let res = try run.run(args: []).outputString()
		XCTAssertEqual(res, "/tmp\n")
		
		let run2 = SPCProcessRunner(executablePath: "/bin/pwd")
